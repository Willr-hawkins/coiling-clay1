...
def profile(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(
             request,
             'Updating profile failed. Please ensure the form is valid.')
    else:
...
def delete_wishlist(request, wishlist_id):
    ...
    if request.method == 'POST':
        ...
        messages.success(request,
                         f'You have successfully deleted your,'
                         '{wishlist.wishlist_name} wishlist.')
        return redirect(reverse('wishlists'))
...
def add_to_wishlist(request, product_id, wishlist_id):
    ...
    if created:
        messages.success(request,
                         f'{product.name} was successfully'
                         'added to {wishlist.wishlist_name}!')
    else:
        messages.info(request,
                      f'{product.name} is already saved to'
...
def remove_from_wishlist(request, product_id, wishlist_id):
    ...
    wishlist_item = get_object_or_404(WishlistItem,
                                      wishlist=wishlist,
                                      product=product)
    wishlist_item.delete()

    messages.success(request,
                     f'{product.name} was successfully removed from your'
                     '{wishlist.wishlist_name} wishlist!')
